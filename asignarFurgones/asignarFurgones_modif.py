...
def readParameters(parametros, furgones, locales, pedidos, lista_incompatibles):
	...
	return parametros, info_furgones, info_locales, info_pedidos, info_lista_incomp
		...
		if num_furgones_llenos > 0 and cant_furgon > 0:
			...
			for r in rango_furgones:
				if cant_furgon > 0:
					...
					info_pedidos.loc[idx,'FRIOS'] = info_pedidos.copy().loc[idx,'FRIOS'] - cap_furgon
					info_pedidos.loc[idx,'TOTAL'] = info_pedidos.copy().loc[idx,'TOTAL'] - cap_furgon
					max_cap_frio.loc[0,'CANTIDAD'] = cant_furgon - 1
					indice = info_cantidad_furgones[(info_cantidad_furgones.TIPO == 'F') & (info_cantidad_furgones.CAPACIDAD == cap_furgon)].copy().index[0]
					info_cantidad_furgones.at[indice, 'CANTIDAD'] = cant_furgon - 1
					num_pre = num_pre + 1
				else:
					break
		...
		if num_furgones_llenos > 0 and cant_furgon > 0:
			...
			for r in rango_furgones:
				if cant_furgon > 0:
					...
					carga = carga.append(data_furgon)
					info_pedidos.loc[idx,'SECOS'] = info_pedidos.copy().loc[idx,'SECOS'] - cap_furgon
					info_pedidos.loc[idx,'TOTAL'] = info_pedidos.copy().loc[idx,'TOTAL'] - cap_furgon
					max_cap_seco.loc[0,'CANTIDAD'] = cant_furgon - 1
					indice = info_cantidad_furgones[(info_cantidad_furgones.TIPO == 'S') & (info_cantidad_furgones.CAPACIDAD == cap_furgon)].copy().index[0]
					info_cantidad_furgones.at[indice, 'CANTIDAD'] = cant_furgon - 1
					num_pre = num_pre + 1
				else:
					break
	...
	for idx,row in recorrerPedido.iterrows():
		for idx1,row1 in recorrer.iterrows():
			...
			if row.TOTAL == row1.CAPACIDAD and cantidad_ini > 0:
				...
				carga = carga.append(data_furgon2)
				info_pedidos = info_pedidos.drop(idx)
				indice1 = cant_furg_frios[(cant_furg_frios.TIPO=='F')&(cant_furg_frios.CAPACIDAD==row1.CAPACIDAD)].index[0]
				cant_furg_frios.at[indice1,'CANTIDAD'] = cantidad_ini - 1
				indice2 = info_cantidad_furgones[(info_cantidad_furgones.TIPO == 'F') & (info_cantidad_furgones.CAPACIDAD == row1.CAPACIDAD)].index[0]
				info_cantidad_furgones.at[indice2, 'CANTIDAD'] = cantidad_ini - 1
				num_pre = num_pre + 1
	...
	for idx,row in recorrerPedido.iterrows():
		for idx1,row1 in recorrer.iterrows():
			...
			if row.SECOS == row1.CAPACIDAD and cantidad_ini > 0:
				...
				info_pedidos.loc[idx,'TOTAL'] = row.TOTAL - row1.CAPACIDAD
				indice1 = cant_furg_secos[(cant_furg_secos.TIPO=='S')&(cant_furg_secos.CAPACIDAD==row1.CAPACIDAD)].index[0]
				cant_furg_secos.at[indice1,'CANTIDAD'] = cantidad_ini - 1
				indice2 = info_cantidad_furgones[(info_cantidad_furgones.TIPO == 'S') & (info_cantidad_furgones.CAPACIDAD == row1.CAPACIDAD)].index[0]
				info_cantidad_furgones.at[indice2, 'CANTIDAD'] = cantidad_ini - 1
				num_pre = num_pre + 1	
...
def asignarNumeroFurgonesOpl(num_furgones_opl,info_locales, info_cantidad_furgones, info_pedidos, param_asignacion):
	...
	solucion_propios = pandas.DataFrame()
	...
	with create_opl_model(model=num_furgones_opl) as opl:
		...
		if opl.run():
			solucion = opl.report
			solucion_propios = solucion['solucion']
			solucion_subcontratar = solucion['solucion_subcontratar']
			#print(solucion)
		else:
			print('error: Número de furgones')
	
	return solucion_propios, solucion_subcontratar
...
def asignarFurgonesOpl(furgones_opl,parametros,info_subzona,pedido_sub,lista_furgones,lista_localFurgon,sub_incompatibles,lista_distancias):
	asignacion = pandas.DataFrame()
...
def main(parametros,furgones, locales, pedidos, lista_incompatibles):
	#pdb.set_trace()
	inicio = time.time()
	
	parametros, info_furgones, info_locales, info_pedidos, info_lista_incomp = readParameters(parametros,furgones, locales, pedidos, lista_incompatibles)
	...
	dist_furgones_propios, dist_furgones_sub = asignarNumeroFurgonesOpl(oplNumFurgones,info_locales, info_cantidad_furgones, info_pedidos, param_asignacion)
	
	if len(dist_furgones_sub) > 0:
		dist_furgones = dist_furgones_propios.append(dist_furgones_sub)
		#Recalculo de info_cantidad con la de subcontratacion
		nueva_cantidad = dist_furgones.groupby(['furgon'], as_index=False).cantidad.sum()
		nueva_cantidad = nueva_cantidad.rename(columns={'furgon':'ID','cantidad':'NUEVA_CANTIDAD'})
		info_cantidad_furgones = info_cantidad_furgones.merge(nueva_cantidad, left_on='CANTIDAD', right_on='CANTIDAD',how='left')
		info_cantidad_furgones = info_cantidad_furgones.drop(columns={'CANTIDAD'})
		info_cantidad_furgones = info_cantidad_furgones.rename(columns={'NUEVA_CANTIDAD','CANTIDAD'})
	else:
		dist_furgones = dist_furgones_propios
	...
	for i in subzonas:
		...
		if len(lista_localFurgon) > max_num :
			...
			for j in num_grupos:
				...
				asignacion, carga_furgon = asignarFurgonesOpl(oplFurgones,parametros,info_grupo,pedido_grupo,lista_furgones,lista_localFurgon,sub_incompatibles,lista_distancias)
			...
			if len(info_subzona) > 1:
				lista_distancias = crearDistancias(info_subzona)
			#pdb.set_trace()
			asignacion, carga_furgon = asignarFurgonesOpl(oplFurgones,parametros,info_subzona,pedido_sub,lista_furgones,lista_localFurgon,sub_incompatibles,lista_distancias)
	...
	carga_furgon_total.to_excel('asignacionFurgon_inicial.xlsx')
	fin = time.time() - inicio
	print('tiempo utilizado: '+str(fin))
if __name__ == "__main__":
	main('parametros_furgones.xlsx','furgones.xlsx','locales.xlsx','pedido_20180313.xlsx','pares_incompatibles.xlsx')
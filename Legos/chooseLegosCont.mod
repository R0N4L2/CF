...
dvar float usePallet[palletIDs] in 0..1;
dvar float useLego[legoIDs] in 0..1;
dvar float assignLegoToPallet[legoPallets] in 0..1;
dvar float esContaminante[palletsWithoutBaseAssigned] in 0..1;
dvar float esSispe[palletsWithoutBaseAssigned] in 0..1;
...
{resultType} resultSet = {<lidx,pidx> | <lidx,pidx> in legoPallets: assignLegoToPallet[<lidx,pidx>] == 1};
tuple baseDualType{
	int pidx;
	float dv;
}
{baseDualType} baseDuals = {};
...
{itemDualType} itemDuals = {};

execute {
	for (var pidx in palletsWithoutBaseAssigned){
		baseDuals.add(pidx,	c_base[pidx].dual);
	}
	for (var cbs in cbSispes){
		if (cbs.sispe == 1){
			itemDuals.add(cbs.cb,cbs.sispe,c_demandaConSispe[cbs.cb].dual);
		} else {
			itemDuals.add(cbs.cb,cbs.sispe,c_demandaSinSispe[cbs.cb].dual);		
		}		
	}
}